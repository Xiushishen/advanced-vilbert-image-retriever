...
def ForwardModelsVal(args, task_cfg, device, task_id, batch, model, task_losses):
    ...
    def process_element(t):
        if isinstance(t, torch.Tensor):
            return t.cuda(device=device, non_blocking=True)
        elif isinstance(t, list):  
            if isinstance(t[0], str):
                max_length = max(len(s) for s in t)
                encoded_strings = [
                    [ord(c) for c in s.ljust(max_length)]
                    for s in t
                ]
                return torch.tensor(encoded_strings).cuda(device=device, non_blocking=True)
            else:
                return torch.tensor(t).cuda(device=device, non_blocking=True)
        else:
            return t

    batch = tuple(process_element(t) for t in batch)
...
def LoadDatasets(args, task_cfg, ids, split="trainval"):
    ...
    for i, task_id in enumerate(ids):
        ...
        print(f"Loaded dataset for task {task}: {len(task_datasets_train[task])} samples")
